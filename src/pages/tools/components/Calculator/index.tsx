...
import {Button, Col, Empty, Input, InputNumber, InputNumberProps, Row, Slider} from 'antd';
...
// 封装计算方法
function calculateExpression(expression, precision: number) {
...
const Calculator: React.FC = () => {
  ...
  const checkTextareaFocus = useCallback(() => {
  ...
  const onClickKeyCode = useCallback((e) => {
    ...
    if (keyCode === '=') {
      try {
        const ret = calculateExpression(content, precision);
        setResult(ret);
  ...
  const onPressEnter = useCallback((e, _precision?: number) => {
    e?.preventDefault?.();
    if (!content.trim()) {
      return warning("请输入计算内容");
    }
    try {
  ...
  const onPrecisionChange: InputNumberProps['onChange'] = (newValue) => {
    setPrecision(newValue as number);
    onPressEnter(undefined, Number(newValue));
  };

  return <div className="calculator-tool-container">
    <div className="tool-operation-container">
      <div className="calc-container">
        <Textarea className="textarea" placeholder="请输入自定义内容进行计算，点击回车显示运算结果" rows={12} value={content} onChange={(e) => {
          setContent(e.target.value);
          if (!e.target.value) {
            setResult('');
          }
        }} onPressEnter={onPressEnter} spellCheck={false} ref={textareaRef}></Textarea>
        <div className={cx('precision-input')}>
          <div>计算精度：</div>
          <Slider
            min={0}
            max={10}
            onChange={onPrecisionChange}
            value={typeof precision === 'number' ? precision : 0}
            style={{
              width: "200px"
            }}
          />
